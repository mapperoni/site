...
- Section extraction: collects headings up to level 2 and the paragraphs that follow them. Headings get stable IDs via `@sindresorhus/slugify`; paragraphs are appended to the most recent section. H3+ content is not indexed.
...
- After adding or editing docs, rerun `node scripts/generate-search-data.mjs` during development so the local search index stays current (the build step already does this for production).
